...
jacobi_mpi.intel: jacobi.f90
	mpiifort -D_MPI -DOVERLAP -o $@ -fpp -march=corei7-avx -O3 -fopenmp $<

jacobi_mpi.intel.nooffload: jacobi.f90
	mpiifort -D_MPI -DOVERLAP -o $@ -fpp -march=corei7-avx -O3 -fopenmp -qno-offload $<

jacobi_mpi.pgi: jacobi.f90
	mpif90 -D_MPI -DOVERLAP -o $@ -Mpreprocess -O4 -Mvect=simd:256 -Mfma -mp $<

jacobi_acc_mpi.pgi: jacobi.f90
	mpif90 -D_MPI -DOVERLAP -o $@ -Mpreprocess -O4 -Mvect=simd:256 -Mfma -acc -ta=tesla:cc35 -Minfo=accel $<
